...
Отсюда отличие в CSS: **правило фокуса не переопределяет `bg`** — у Light он остаётся белым, тогда как Default при фокусе уходит в `primary-container-base`. Меняются рамка и цвета текста. Рамка, hover, active, кольцо — как у Default.
