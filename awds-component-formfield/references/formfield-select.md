...
У селекта внутри поля пустой `<option value="">` показывает плейсхолдер и красится ролью `placeholder`. Не путать с `awds-component-select-combi`, где пустой пункт обязан быть без текста: там его роль играет плавающая метка, а здесь подпись стоит снаружи и текст в пустом пункте уместен.
