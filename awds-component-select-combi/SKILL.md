...
- **Пустой `<option value="">` без текста** — на нём держится состояние «ничего не выбрано», по которому метка возвращается на центр. Текст в него писать нельзя: подпись уже нарисована меткой, второй текст наложился бы на неё. В раскрытом списке этот пункт скрыт (`option[value=""] { display: none }`), поэтому классы `list-item` ему не нужны — показывать нечего. Если значение выбрано всегда (пустого варианта нет вовсе) — метка просто стоит наверху с самого начала, это нормальный сценарий.
...
**Классы `list-item list-item-transparent` несёт каждый пункт с текстом.** Вид пункта целиком приходит из [`awds-component-list-item`](../awds-component-list-item/SKILL.md): голый `<option>` под `base-select` остаётся и без него, и без UA-галочки — её компонент гасит у всех пунктов, потому что в макете выбранный отмечен фоном и рамкой. Значит голый выбранный пункт не отмечен ничем. Там, где `base-select` не поддержан, классы просто игнорируются.
