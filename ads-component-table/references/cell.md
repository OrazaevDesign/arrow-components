...
Подключи `table.css` (база `.cell` + обёртка `.ads-table`) и нужные `cell-*.css`. База потребляет приватные аккумуляторы `--ads-cell-bg / -border / -color / -description`, варианты их заполняют ролями.
...
Active = Rest. Focus = Rest + обводка. Disabled = Rest + `opacity: var(--ads-opacity-40)`.

## Раскладки таблицы

Ячейки кладутся в одну из двух раскладок (Figma-нода `2038:1350`):

- `.ads-table--default` — сетка: все ячейки с границей (`border-collapse`), шапка залита. Паддинг равномерный `--ads-space-3`.
- `.ads-table--light` — безрамочная: только `border-bottom` между строками (`surface-on-lowest`), ячейки прозрачные, первый столбец flush-влево (`padding-inline-start: 0`).

Широкую таблицу оборачивай в `.ads-table-scroll` (нативный `overflow-x:auto`).

## Заметки

- **`cell-head` — semibold** (`--ads-font-weight-semibold`), тело (`cell-light`/`cell-default`) — `regular`. Подтверждено нодой таблицы (head = `Inter:Semi_Bold`).
