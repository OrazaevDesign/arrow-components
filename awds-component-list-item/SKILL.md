...
description: >
  Верстка строки списка (list item, пункт меню, пункт выпадающего списка, вкладка
  таббара, плитка выбора вариации товара) ArrowDS — HTML-разметка и CSS на токенах
  дизайн-системы. 12 вариантов. Используй ВСЕГДА при: пунктах меню и навигации,
  строках результатов поиска и автодополнения, пунктах выпадающего списка и попапа,
  выбранных и невыбранных строках списка, вкладках нижней панели, выборе вариации
  товара (цвет, размер, комплектация), строке с подзаголовком-описанием,
  вопросах о стилях list-item / list-item__prefix / list-item__content /
  list-item__title / list-item__description / list-item__suffix / list-item__check,
  о ролях list/unselected / list/selected / list/unselected-transparent /
  list/selected-secondary / list/unselected-tabbar / list/unselected-menu /
  list/unselected-variation, выборе размера строки (высота 20/24/32/36/40/48/52px).
  Это строительный кирпич списков: сам список (панель, скролл, клавиатура) собирается
  вокруг него. Для нативного выпадающего списка целиком — awds-component-select,
  для текстового ввода — awds-component-input, не этот скилл.
---
...
**Зазор заголовок → описание — 2px, фикс на всех размерах.** Это `--awds-space-0-5`, а **не** `rectangle-{N}-gap`: тот идёт 2/2/4/6/6/6/8 и на размере 600 дал бы 8 вместо 2. На 100 и 50 значения случайно совпадают — там ошибку не заметить.
...
| Зазор заголовок/описание | `var(--awds-space-0-5)` | Figma: `space/0,5`, фикс |
...
| Opacity для disabled | `var(--awds-opacity-40)` | css-global |
