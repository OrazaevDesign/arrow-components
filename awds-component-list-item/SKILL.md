...
description: List Item ArrowDS (.list-item).
---
...
**В макете это переключается руками — `:has()` в Figma нет.** Мастера нарисованы со
включённым префиксом, поэтому у вложенного `content-list` стоит `padding=off`: отступ даёт
слот. Выключаешь `show-prefix` и `show-suffix` — ставь `padding=on`, иначе текст ляжет
вплотную к краю строки. В CSS этого делать не нужно: там правило срабатывает само.

Промах заметен не сразу: на стенде компонентов 24.09.2026 так стояли все двенадцать
вариантов, и строки выглядели «съехавшими» ровно на `text-gap` (18px у ступени 600).
...
Пересчитано 25.09.2026 после того, как студия перевела часть ячеек на другие роли, а палитра
сдвинулась — прежние замеры (3.69 / 3.95 / 2.06) относились к старым значениям и больше не верны:
